...
/**
 * 当用户选择机构后将触发organInfoServiceController，此控制器负责与后台的默认会议接口进行数据交互处理（mLogin.do?action=defaultMeeting），主要包括以下操作：
 * 1、接口请求参数封装； 2、接口请求； 3、接口数据解析； 4、接口数据保存本地数据库.
 *
 * @author zhaojunfeng, chenxi
 */
public class OrganInfoServiceController {

    private static final Logger logger = LoggerFactory.getLogger(OrganInfoServiceController.class);
    ...
    private SyncParamsService syncParamsService;

    /**
     * initialize方法是organInfoServiceController业务逻辑的触发点和唯一入口.
     */
    public void initialize() throws ApplicationException, Exception {
        ...
        syncParamsService = new SyncParamsService();
        //封装参数
        String jsonRequestString = buildJsonRequestString(GlobalStaticConstant.GLOBAL_ORGANINFO_OWNER_USERID, GlobalStaticConstant.GLOBAL_ORGANINFO_ORGANIZATIONID, "zh_CN", getUpdateDate());
        //访问接口
        logger.debug("调用defaultMeeting接口开始时间：{}", DateUtil.formatFullDateTime(new Date()));
        String response = HttpClientUtil.getInstance().getResponseBodyAsString(ServeIpConstant.selectOrganServicePath(), jsonRequestString);
    ...
    public String getUpdateDate() throws ApplicationException {
        ...
        String updateDate = "";
        //根据用户id和机构id查出对应的机构
        organInfoList = organInfoService.queryOrganInfosByMap(GlobalStaticConstant.GLOBAL_ORGANINFO_ORGANIZATIONID, GlobalStaticConstant.GLOBAL_ORGANINFO_OWNER_USERID);
    ...
    public void handleSyncParams(Map dataMap) throws ApplicationException {
        ...
        if (!syncParamsMap.isEmpty()) {
            ...
            if (!syncParamsList.isEmpty()) { //如果数据已存在，更新这条数据
                ...
                syncParamsService.saveOrUpdateSyncParams(setSyncParamsProperties(syncParams, syncParamsMap));
    ...
    public void handleMisc(Map dataMap) throws ApplicationException {
        ...
        if (!organInfoList.isEmpty()) {
            ...
            organInfoService.saveOrUpdateOrganInfo(organInfo);
        }
    }
}
